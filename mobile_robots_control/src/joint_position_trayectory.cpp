/*
@description:
Simple example to command joints of a robot using joint_trajectory_controller

USAGE:
In different terminals:
$ ros2 launch mobile_robots_control joint_trajectory_launch.py
	# Launch the simulated robot with 2 joints
$ ros2 run mobile_robots_control joint_position_trayectory
	# Run this node
...
int main(int argc, char * argv[])
{
  ...
  std::vector<std::string> joint_names = {"pos0_joint", "pos1_joint"};
  //Joints velocities are not correctly published, but position limits ARE respected

  std::vector<trajectory_msgs::msg::JointTrajectoryPoint> points;
  trajectory_msgs::msg::JointTrajectoryPoint point;
  point.time_from_start = rclcpp::Duration::from_seconds(0.0);  // start
  ...
  point2.time_from_start = rclcpp::Duration::from_seconds(5.0); //incremental time
  point2.positions.resize(joint_names.size());
  point2.positions[0] = -1.0;
  point2.positions[1] = -3.1;

  trajectory_msgs::msg::JointTrajectoryPoint point3;
  point3.time_from_start = rclcpp::Duration::from_seconds(15.0); //incremental time
  point3.positions.resize(joint_names.size());
  point3.positions[0] = 1.0;
  point3.positions[1] = 3.1;

  trajectory_msgs::msg::JointTrajectoryPoint point4;
  point4.time_from_start = rclcpp::Duration::from_seconds(25.0); //incremental time
